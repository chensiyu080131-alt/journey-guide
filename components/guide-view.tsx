...
export function GuideView({ guide }: GuideViewProps) {
  return (
    <div className="space-y-10 animate-fade-in">
      {guide.routeIntro && (
        <div className="rounded-3xl bg-charcoal p-6 sm:p-8 text-white">
          <p className="text-xuncheng-400 text-xs font-medium tracking-widest uppercase mb-3">
            {guide.entryType === '书籍' ? '书本里的路' : guide.entryType === '人物' ? '一个人的足迹' : '这条路的故事'}
          </p>
          <p className="text-sm sm:text-base leading-relaxed text-white/80">{guide.routeIntro}</p>
          <div className="mt-4 flex flex-wrap gap-2">
            <Badge variant="default">{guide.province}</Badge>
            {guide.relatedBook && <Badge variant="文化">{guide.relatedBook}</Badge>}
            {guide.relatedCharacter && <Badge variant="体验">{guide.relatedCharacter}</Badge>}
          </div>
        </div>
      )}
      ...
      {guide.tips && guide.tips.length > 0 && (
        <Card className="p-5 rounded-2xl border-ink-100">
          <h3 className="font-bold text-sm text-charcoal mb-3">旅行贴士</h3>
