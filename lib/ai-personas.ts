...
const MOCK_CHAT: Record<Persona, string> = {
  文人风骨: '（书灵·文人风骨）此地山水，旧时文人多有题咏。您若循原文而行，脚下每一步皆是纸上曾读之句。欲问何处先往，老夫以为——晨光里的那一处最宜。〔未配置 LLM_API_KEY〕',
  市井幽默: '（书灵·市井幽默）要我说啊，这条道儿最妙的不是景，是"味儿"！您猜怎么着——本地人天不亮就去排那口早面了。跟着书走，也别忘了跟着鼻子走。〔未配置 LLM_API_KEY〕',
  赛博诗人: '（书灵·赛博诗人）\n旧墨在数据流里复活，\n一行原文，点亮一段坐标。\n你沿着字里的光走，\n现实便是被重新渲染的诗。\n〔未配置 LLM_API_KEY〕',
}

const MOCK_ASSOCIATION =
  '【文学密码·演示】看似无关的两处，其实系于同一条文脉：一处以"归隐"入诗，一处以"离乱"著史，而它们共享着同一片水域的意象——水既是退隐的屏障，也是乱世的通路。前人于此写"藏"，后人于此写"走"，一藏一走，正是这座城在文学中的两种呼吸。〔未配置 LLM_API_KEY〕'
