...
originalText 必须是书中真实句子。`
  try {
    ...
    return (parsed.location对照 ?? []).filter(
      loc => loc.bookName?.trim() && loc.originalText?.trim()
    )
  } catch {
    return []
...
function buildMockResponse(req: BookGuideRequest, resolvedCity: string): BookGuideResponse {
  const base = getMockGuide(resolvedCity, req.days, req.interests, req.budget)
  const guide: Guide = {
    ...base,
    id: `book-${Date.now()}`,
    ...
    routeIntro:
      base.routeIntro ||
      `跟着${req.author}的《${req.bookTitle}》，在${resolvedCity}寻找文字里的真实坐标。`,
  }
...
export async function generateBookGuideClient(
  req: BookGuideRequest
  ...
  try {
    ...
    console.error('书籍攻略生成失败:', error)
    const message =
      error instanceof Error ? error.message : '攻略生成失败'
    throw new Error(message.includes('LLM') ? message : `攻略生成失败：${message}`)
  }
}
