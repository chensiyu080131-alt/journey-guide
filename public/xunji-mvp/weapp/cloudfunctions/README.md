...
3. 在云开发控制台新建 **4 个集合**：`poems`、`spots`、`routes`、`collections`（另含 `checkins` 由打卡函数自动写入，可选预建）。
...
- `checkin`：写入一次打卡（`checkins` 集合，按 openid+spotId 去重，记录 `city`），合并到 `collections.unlockedSpotIds`，**返回最新 `unlockedSpotIds`**。
- `stats`：聚合 `checkins`，返回 B 端真实统计 `{ participants(去重用户), totalCheckins, heat(各景点), city(各城市) }`，供数据面板在云端模式渲染真实数据（本地模式面板用模拟兜底）。
