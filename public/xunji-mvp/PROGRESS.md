...
## ▎执行者续做（Turn C 任务书 T0–T6，2026-07-28 起）
### 任务0：线上核对（已做）
- 实测 http://47.109.91.112:8080 ：首页 200、/route/... 200、未知 route 404(品牌页)。路由页 chunk 含 `webapi.amap.com` 模板但无注入 Key → 当前走零依赖 SVG 回退（不卡死）。
